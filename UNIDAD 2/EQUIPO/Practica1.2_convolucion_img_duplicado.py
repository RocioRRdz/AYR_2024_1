...
for filas in range(alto+2): #pixeles extra de la primera y ultima fila
    new_fila = []
    for columnas in range(largo+2): #pixeles extra de la primera y ultima columna
        ...
        for f_kernel in range(len(kernel)): # 0 1 2
            for c_kernel in range(len(kernel)): # 0 1 2
                pixelConvulucionado += (kernel[f_kernel][c_kernel]
                                        * img_a_convolucinar[min(max(filas + (f_kernel-1), 0), alto - 1)]
                                        [min(max(columnas + (c_kernel-1), 0), largo - 1)])
